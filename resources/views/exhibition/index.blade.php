            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="text-center text-danger p-2">
                        ...
                        <h4 class="text-white font-wight-bold ml-5">Voters' Register Exhibition</h4>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-2">
                <form class="form-horizontal" method="POST" action="{{ route('voter.exhibition.submit') }}">
                    ...
                    <div class="form-group">
                        <label for="password">Enter {{ $election->password_title ?? 'Password' }}</label>
                        <input type="text" name="password" class="form-control @error('password') is-invalid @enderror"
                            id="password" maxlength="6"
                            placeholder="{{ $election->password_title ?? 'Password' }}"
                            autofocus required autocomplete="password">
                        @error('password')
                    ...
                    <div class="form-group">
                        <label for="username">Enter {{ $election->username_title ?? 'Email or Phone' }}</label>
                        <input type="text" name="username" class="form-control @error('username') is-invalid @enderror"
                            id="username"
                            placeholder="{{ $election->username_title ?? 'Email or Phone' }}"
